...
-(void) iphoneXFeaturesReleaseDate:(NSString *)releaseDate andManufactureBy:(NSString *)manufactureBy andDisplay:(NSString *)display andProcessor:(NSString *)processor andRAM:(NSString *)RAM andStorageInGB:(NSString *)storageInGB andRearCamInMP:(NSString *)rearCamInMP andOS:(NSString *)OS andFrontCam:(NSString *)frontCam
                             andBatCap:(NSString *)batteryCapacity  bluetooth:(float)bluetooth weight:(NSString *)weight;
{
    ...
    NSLog(@" IphoneX is manufactured by %@ and released on %@",manufactureBy,releaseDate);
    ...
    NSLog(@"New Feature:wireless charging");
    NSLog(@"IphoneX unlocked with FaceID");
    NSLog(@"No home button and no touchID");
    NSLog(@"An OLED screen with highest pixel density");
    NSLog(@"Added anew tele photo lens which works better in low light conditions");
