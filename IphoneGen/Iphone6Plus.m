...
@implementation Iphone6Plus
-(void) iphone6PlusFeaturesReleaseDate:(NSString *)releaseDate andManufactureBy:(NSString *)manufactureBy andDisplay:(NSString *)display andProcessor:(NSString *)processor andRAM:(NSString *)RAM andStorageInGB:(NSString *)storageInGB andRearCamInMP:(NSString *)rearCamInMP andOS:(NSString *)OS andFrontCam:(NSString *)frontCam
                          andBatCap:(NSString *)batteryCapacity bluetooth:(float)bluetooth weight:(NSString *)weight;
{
    ...
    NSLog(@" Iphone6 plus is manufactured by %@ and released on %@",manufactureBy,releaseDate);
    ...
    NSLog(@"weight:%@",weight);
    NSLog(@"Introduced NFC forApple pay and other services");
    NSLog(@"advanced camera with optical image stabilization");
